...
This module contains content and sample helper functions.
"""
...
def get_sample_by_euid(
    ...
    try:
        ...
        if not include_deleted:
            query = query.filter(base.classes.content_instance.is_deleted.is_(False))
...
def create_aliquot(
    ...
    existing_aliquots = (
        session.query(base.classes.content_instance)
        .filter(
            base.classes.content_instance.json_addl.contains(
...
def get_sample_lineage(
    ...
    try:
        children = (
            ...
            .filter(
                base.classes.content_instance.json_addl.contains(
                    {"parent_sample_euid": sample.euid}
                )
            )
            .all()
        )
