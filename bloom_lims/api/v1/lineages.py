...
async def list_lineages(
    ...
    try:
        ...
        query = (
            ...
            .filter(lineage.is_deleted.is_(False))
        ...
        if parent_euid:
            parent = (
                ...
                .filter(bdb.Base.classes.generic_instance.euid == parent_euid)
                .first()
            )
            if parent:
                query = query.filter(
                    bdb.Base.classes.generic_instance_lineage.parent_instance_uid
                    == parent.uid
                )
        ...
        if child_euid:
            child = (
                bdb.session.query(bdb.Base.classes.generic_instance)
                .filter(bdb.Base.classes.generic_instance.euid == child_euid)
                .first()
            )
            if child:
                query = query.filter(
                    bdb.Base.classes.generic_instance_lineage.child_instance_uid
                    == child.uid
                )
...
async def create_lineage(
    ...
    try:
        ...
        if not parent:
            raise HTTPException(
                status_code=404, detail=f"Parent not found: {data.parent_euid}"
            )

        child = bo.get_by_euid(data.child_euid)
        if not child:
            raise HTTPException(
                status_code=404, detail=f"Child not found: {data.child_euid}"
            )

        # Create lineage
        lineage = bo.create_lineage(
            parent, child, relationship_type=data.relationship_type or "generic"
        )
        bdb.session.commit()
...
async def delete_lineage(
    ...
    try:
        ...
        lineage = (
            ...
            .filter(lineage_model.is_deleted.is_(False))
            .filter(
                lineage_model.polymorphic_discriminator == "generic_instance_lineage"
            )
        ...
        if not lineage:
            raise HTTPException(
                status_code=404, detail=f"Lineage not found: {lineage_euid}"
            )
