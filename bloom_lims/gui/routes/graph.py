...
def _build_graph_elements_for_start(
    bobj: BloomObj, start_euid: str, depth: int
) -> tuple[list, list]:
...
async def api_graph_data(
    ...
    try:
        ...
        nodes, edges = _build_graph_elements_for_start(
...
async def api_external_graph(
    ...
    if not ref.graph_expandable:
        raise HTTPException(
            status_code=424, detail=ref.reason or "External graph unavailable"
        )
...
async def api_external_graph_object(
    ...
    if not ref.graph_expandable:
        raise HTTPException(
            status_code=424, detail=ref.reason or "External object unavailable"
        )

    try:
        return AtlasService().client.get_graph_object_detail(
            euid=euid, tenant_id=ref.tenant_id
        )
    except (AtlasDependencyError, AtlasClientError) as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    except Exception as exc:
...
async def api_create_lineage(
    ...
    relationship_type = (
        data.get("relationship_type") or "generic"
    ).strip() or "generic"

    if not parent_euid or not child_euid:
        raise HTTPException(
            status_code=400, detail="parent_euid and child_euid are required"
        )
    if parent_euid == child_euid:
        raise HTTPException(
            status_code=400, detail="parent_euid and child_euid must differ"
        )
    ...
    parent_obj = (
        ...
        .filter(instance_cls.euid == parent_euid, instance_cls.is_deleted.is_(False))
        .first()
    )
    child_obj = (
        bobj.session.query(instance_cls)
        .filter(instance_cls.euid == child_euid, instance_cls.is_deleted.is_(False))
        .first()
    )
