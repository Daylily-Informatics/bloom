...
class TapdbUserAPITokenRepository:
    ...
    def create_token(
        ...
        *,
        ...
        token_id = ensure_uuid_property(token_properties, "id")
    ...
    def get_latest_revision(self, token_id: uuid.UUID) -> UserTokenRevisionRecord | None:
    ...
    def log_usage(
        ...
        *,
        ...
        ensure_uuid_property(usage_properties, "id")
    ...
    def count_usage(self, *, token_id: uuid.UUID) -> int:
        return len(self.get_usage_logs(token_id=token_id, limit=1_000_000))

    def _find_token_instance(self, token_id: uuid.UUID) -> generic_instance | None:
        for token_instance in self._instances_for_template(TOKEN_TEMPLATE_CODE):
            props = self._props(token_instance)
            if resolve_public_id(token_instance, props) == token_id:
                return token_instance
        return None

    def _to_token(self, token_instance: generic_instance) -> UserTokenRecord | None:
        props = self._props(token_instance)
        token_id = resolve_public_id(token_instance, props)
    ...
    def _to_revision(self, revision_instance: generic_instance) -> UserTokenRevisionRecord | None:
        ...
        token_id = parse_uuid(props.get("token_id"))
        revision_id = resolve_public_id(revision_instance, props)
        ...
        return UserTokenRevisionRecord(
            ...
            revoked_by=parse_uuid(props.get("revoked_by")),
    ...
    def _to_usage(self, usage_instance: generic_instance) -> UserTokenUsageRecord | None:
        ...
        usage_id = resolve_public_id(usage_instance, props)
        token_id = parse_uuid(props.get("token_id"))
        user_id = parse_uuid(props.get("user_id"))
