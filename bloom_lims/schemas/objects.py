...
class ObjectBaseSchema(BloomBaseSchema):
    ...
    type: str = Field(
        ...
        min_length=1,
        max_length=100,
        description="Object type (e.g., sample, plate)",
    )
    subtype: Optional[str] = Field(None, max_length=100, description="Object subtype")
    category: str = Field(
        default="instance", max_length=50, description="Object category"
    )
    json_addl: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Additional JSON data"
    )
...
class ObjectCreateSchema(ObjectBaseSchema):
    ...
    parent_euid: Optional[str] = Field(
        None, description="Parent object EUID for lineage"
    )
    lineage_euid: Optional[str] = Field(None, description="Lineage EUID to attach to")
    container_euid: Optional[str] = Field(
        None, description="Container EUID for placement"
    )
    container_position: Optional[str] = Field(
        None, description="Position in container (e.g., A1)"
    )
    count: int = Field(
        default=1,
        ge=1,
...
class ObjectUpdateSchema(BloomBaseSchema):
    ...
    name: Optional[str] = Field(
        None, min_length=1, max_length=500, description="Object name"
    )
    json_addl: Optional[Dict[str, Any]] = Field(
        None, description="Additional JSON data (merged)"
    )
    ...
    created_dt: Optional[datetime] = Field(
        None, description="Creation datetime (admin only)"
    )
...
class ObjectResponseSchema(ObjectBaseSchema, TimestampMixin):
    ...
    polymorphic_discriminator: Optional[str] = Field(
        None, description="Type discriminator"
    )
...
class ObjectQueryParams(BloomBaseSchema):
    ...
    include_deleted: bool = Field(
        default=False, description="Include soft-deleted objects"
    )
