...
import bloom_lims.domain.workflows as workflows_mod  # noqa: E402
from bloom_lims.domain.workflows import BloomWorkflowStep  # noqa: E402
...
def test_iter_active_child_instances_filters_deleted_and_type():
    ...
    deleted_child = _obj(
        "DEL-CHILD", category="container", type_name="well", is_deleted=True
    )
    ...
    found = list(
        s._iter_active_child_instances(parent, category="container", type_name="well")
    )
...
def test_default_queue_name_handles_special_tokens():
    s = _subject()
    assert (
        s._default_queue_name("input-gdna-normalization-eligible")
...
def test_get_plate_wells_in_address_order_and_destination_resolution():
    ...
    w2 = _obj(
        ...
        category="container",
        type_name="well",
        cont_address={"row_idx": "2", "col_idx": "1"},
    )
    w1 = _obj(
        "W1",
        category="container",
        type_name="well",
        cont_address={"row_idx": "1", "col_idx": "2"},
    )
    w0 = _obj(
        "W0",
        category="container",
        type_name="well",
        cont_address={"row_idx": "1", "col_idx": "1"},
    )
    plate = _obj("PLATE1", category="container", type_name="plate")
    ...
    s._resolve_well_by_plate_and_address = lambda plate_euid, addr: _obj(
        f"{plate_euid}.{addr}"
    )
...
def test_parse_mapping_rows_and_quant_rows_validation():
    ...
    assert s._parse_mapping_rows(mapping_ds) == [
        ("PLATE1.A1", "A1"),
        ("PLATE1.B1", "B1"),
    ]

    with pytest.raises(Exception, match="Invalid mapping row"):
        s._parse_mapping_rows(
            {
                "captured_data": {
                    "mapping_csv_text": "source_ref,destination_ref\nONLYSOURCE\n"
                }
    ...
    assert s._parse_quant_rows(quant_ds) == [
        ("PLATE1", "A1", 1.5),
        ("PLATE1", "B1", 2.0),
    ]
    ...
    with pytest.raises(Exception, match="Quant value must be float"):
        s._parse_quant_rows(
            {"captured_data": {"quant_csv_text": "PLATE1,A1,not-a-float\n"}}
        )
...
def test_get_library_prep_targets_from_properties_or_defaults():
    ...
    wf_with_string.json_addl["properties"]["library_prep_queue_subtypes"] = (
        "ont-libprep-eligible"
    )
...
def test_get_or_create_queue_step_returns_existing_without_creation():
    ...
    s._get_workflow_steps = lambda _wf, subtype=None: (
        [existing] if subtype == "target" else [existing]
    )
...
def test_get_or_create_queue_step_creates_new_step_with_incremented_number():
    s = _subject()
    highest = _obj(
        "STEP-HIGH", category="workflow_step", subtype="other", step_number="4"
    )
    call_state = {"count": 0}
    ...
    s.create_instance_by_code = lambda layout, payload: (
        captured.update({"layout": layout, "payload": payload}) or created
    )
    s.create_generic_instance_lineage_by_euids = lambda parent, child: captured.update(
...
def test_fill_plate_from_sources_undirected_and_directed_error_paths():
    s = _subject()
    queue_step = _obj(
        "Q1", category="workflow_step", subtype="input-gdna-normalization-eligible"
    )
    plate = _obj("PLATE1", category="container", type_name="plate")
    ...
    with pytest.raises(Exception, match="Destination well already occupied"):
        s._fill_plate_from_sources(
            queue_step, plate, ["A1"], directed_mapping=[("A1", "A1")]
        )
...
def test_fill_plate_from_sources_rejects_insufficient_wells():
    ...
    s._create_output_gdna_content = lambda normalized=False: _obj(
        "OUT", category="content", type_name="sample"
    )
...
def test_create_output_gdna_content_sets_normalization_flags(monkeypatch):
    ...
    monkeypatch.setattr(
        workflows_mod, "flag_modified", lambda _obj, field: touched.append(field)
    )
